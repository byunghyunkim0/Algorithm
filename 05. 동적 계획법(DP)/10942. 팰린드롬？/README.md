...
2차원 배열을 만들어서 모든 문자에 대해서 팰린드롬이 성립하는지 저장<br>
질문을 받을 때마다 팰린드롬인지 확인하는게 아니라 모든 팰린드롬을 확인하고 S와 E에 해당하는 2차원 배열의 값을 출력