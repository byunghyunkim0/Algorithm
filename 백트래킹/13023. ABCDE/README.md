...
인접 리스트를 사용하여 사람들 간의 친구 관계의 정보를 저장(양방향)

0 ~ (N - 1)번 각각의 사람을 시작으로 DFS를 이용하여 탐색을 진행

> BFS를 사용하여 탐색을 진행한다면, 친구 관계에서 사이클이 있다면 문제가 발생한다.

DFS를 통해 서로 다른 친구의 관계가 5명을 만족하면 DFS를 종료 후 결과 출력