...
피연산자가 아닌 알파벳은 바로 출력하고 연산자에 대해서만 스택에 넣으면서 판단<br>
<code>(</code>가 나오면 무조건 스택에 넣고 <code>)</code>가 나올때까지 스택에서 빼지않는다.<br>
각각의 피연산자들은 우선순위를 통해서 스택에서 뺀다<br>
<code>(</code>, <code>)</code>는 가장 높고, 다음은 <code>*</code>와 <code>/</code>, 다음은 <code>+</code>와 <code>-</code>순서로 우선순위가 설정된다.<br>
우선순위가 같으면 스택에 저장하고, 들어오는 피연산자의 우선순위가 높으면 우선순위가 같거나 높은 것이 나올때까지 스택에서 제거한다.