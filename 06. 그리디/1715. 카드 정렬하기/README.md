...
최소한의 비교를 하려면 작은 값부터 순서대로 비교하는 것이 가장 적음<br>
카드 묶음은 정렬되어 입력되지 않기때문에 우선순위 큐를 활용하여 입력값을 받는다.<br>
우선순위 큐를 이용하면 가장 작은 값부터 데이터를 가져올 수 있음<br>
작은 값 2개를 뽑고 해당 값의 합을 다시 우선순위 큐에 넣어서 다시 작은 값 2개를 가져오고 더한 값들을 구함<br>