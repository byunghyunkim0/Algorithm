...
자기 자신을 가르키거나 사이클을 만들 수 있는지 확인하는 문제<br>
모든 학생에서 DFS를 이용하여 해당 학생과 연결된 다른 학생에 방문처리를 진행<br>
해당 학생의 정보가 0이 아니면 이미 방문을 했기 때문에 해당 학생에 대해 처리를 진행했다는 의미<br>
처음 방문이라면 1을 넣고 DFS를 진행 ( 1은 해당 위치와 연결된 다른 학생들의 수 )<br>
만약 해당 학생과 연결된 학생의 값이 0이아닌 다른 수라면 이미 방문을 했다는 뜻이기 때문에 사이클이 이루어졌다는 의미<br>
두 값의 차이를 이용하면 사이클의 크기를 알수있기 때문에 전체 학생 수에서 두 값의 차이를 빼면서 어느 팀에도 속하지 않는 학생들의 수를 계산 할 수 있다.