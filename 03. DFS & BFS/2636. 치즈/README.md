...
(0, 0)은 치즈가 존재하지 않기 때문에 해당 위치에서부터 BFS를 이용하여 치즈가 없는 칸을 탐색<br>
치즈가 없는 칸 주위에 치즈가 있다면 해당 칸들을 저장 ( 바로 지우면 문제가 발생 )<br>
저장된 치즈 칸들을 한번에 삭제<br>
남아있는 치즈의 개수를 저장하고 모든 치즈가 사라지기 전의 치즈의 개수를 출력