...
모든 정점에서 BFS를 이용<br>
모든 정점에 대해서 0으로 초기화를 한 후 BFS를 이용하여 처음 방문하는 정점을 1로 변경 ( 해당 정점이 0이고 방문을 하지 않았어야함 )<br>
방문한 정점을 통해 방문할 수 있는 모든 정점에 대한 정보를 queue에 넣으면서 BFS를 진행<br>
이전에 방문했던 값과 비교해서 1과 -1로 값을 변경<br>
만약 이전과 같은 값이 나온다면 해당 그래프는 이분 그래프가 아님