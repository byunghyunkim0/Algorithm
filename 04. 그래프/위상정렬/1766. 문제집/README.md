...
연결 리스트를 이용하여 그래프의 정보를 저장 ( 단방향 그래프 )<br>
들어오는 경로의 개수를 세기위해서 그래프 정보를 받을 때 도착 지점 노드에 해당하는 배열에 +1을 해줌<br>
이 문제는 가능하면 쉬운 문제를 풀어야하기 때문에 정점의 번호가 작은 순서로 출력해야한다. 그래서 우선순위 큐를 활용하여 문제를 풀 수 있는 번호 중 작은 번호를 뽑는다<br>
degree배열 값에 0인 정점에 정보를 우선순위 큐에 삽입 ( 해당 문제를 풀 수 있다는 의미 )<br>
우선순위 큐에서 정점의 정보를 뽑고 해당 정점과 이웃한 정점에 해당하는 degree배열의 값을 1씩 줄인다.<br>
만약 이웃한 정점의 degree 값이 0이라면 우선순위 큐에 넣어서 정점에 방문할 준비를 한다.