...
우선순위 큐를 사용하여 적은 비용의 위치를 가져온다.<br>
가져온 위치가 이미 방문을 해서 해당 위치의 비용이 작다면 continue를 이용하여 다음 위치로 넘어간다.<br>
방문하지 않았다면 해당 위치에서 갈수있는 곳을 탐색하고 해당 위치의 비용에 +1 비용과 위치 정보를 queue에 넣는다.

> 해당 문제는 비용이 모두 1이기 때문에 BFS / DFS를 이용하여 거리를 구할수도 있다.