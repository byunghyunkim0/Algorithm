...
2차원 배열로 각각의 위치에 대한 비용이 주어짐<br>
모든 위치에 대해서 비용을 저장하기 위한 2차원 배열을 생성<br>
초기 위치는 ( 0, 0 )이기 때문에 해당 위치의 비용과 위치에 대한 정보를 우선순위 큐에 삽입<br>
비용을 기준으로 적은 정보를 가져오고 만약 가져온 비용과 해당 위치의 비용을 비교했을때 위치의 비용이 적으면 다음이로 넘어감<br>
다른 Node의 정보를 가져와 인접해있는 위치에 대해서 방문하고 만약 해당 위치의 가격이 현재의 가격과 비용의 합이 크다면 우선순위 큐에 다음 위치에 대한 정보를 넣고 2차원 비용 배열을 갱신<br>
마지막 위치 <code>cost[n - 1][n - 1]</code>의 값을 출력