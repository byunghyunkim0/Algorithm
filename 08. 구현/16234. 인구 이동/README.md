...
BFS / DFS를 사용하여 해당 나라의 인구와 조건에 부합하는 주위의 나라에 대한 정보를 work 큐를 이용하여 관리<br>
모든 나라에 대해서 BFS / DFS를 마치면 인구 이동을 시작<br>
BFS / DFS 한번 끝나고 하면 변경된 인구때문에 다른 결과를 가져올 수 있음<br>
work 큐에 있는 작업들을 모두 끝내고 모든 나라에 대해서 인구이동이 일어나는지 확인한후 인구이동이 없다면 반복문을 중지