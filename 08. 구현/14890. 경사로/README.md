...
가로로 보는 경우와 세로로 보는 경우를 전부 확인하기 위해서 해당 좌표에 경사로를 설치 유무를 저장<br>
해당 위치의 값과 다음 확인하는 위치의 값의 차이가 1보다 크다면 경사로를 설치할수 없음<br>
해당 위치값과 다음 위치 값을 비교했을 때 해당 위치 값이 작다면 작은 곳에 경사로를 설치해야한다. 경사로의 길이를 이용하여 해당 위치에서 부터 왼쪽, 위쪽으로 경사로가 들어갈수 있는지 확인<br>
경사로가 들어갈 수 있으면 경사로 설치를 진행하고 저장<br>
불가능한 길의 개수를 세고 전체 경우에서 뺌