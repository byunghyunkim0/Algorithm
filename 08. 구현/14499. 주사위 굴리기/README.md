...
일반적인 주사위가 아닌 주사위의 숫자가 변경될수도 있고 없을 수도 있으므로 주사위를 나타내는 배열을 선언([위, 옆, 옆, 옆, 옆, 아래])<br>
해당 주사위를 굴렸을 때 변경되는 면을 배열에서 swap을 통해 변경<br>
아래 부분의 숫자와 맵을 비교하며 이동을 구현